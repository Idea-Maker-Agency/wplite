...
function wplite_theme_page_templates(array $page_templates, WP_Theme $theme, WP_Post | null $post): array
{
  ...
  if ($templates_dir) {
    $templates_subdir = array_filter($templates_dir, function ($subdir) use ($templates_path) {
      if (in_array($subdir, ['.', '..'])) return false;

      return is_dir($templates_path . '/' . $subdir);
    } );

    $templates_subdir_keys = array_map(function ($value) {
      return 'templates/' . $value . '/' . $value . '.php';
    }, $templates_subdir);

    $templates_subdir_names = array_map(function ($value, $key) {
    ...
    if (! empty($templates_subdir_items)) {
...
function wplite_theme_page_templates_resources()
{
