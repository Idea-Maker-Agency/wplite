...
/**
 * Template parts stylesheets.
 *
 * @since 1.0.0
 *
 * @return void
 */
add_action( 'wp_enqueue_scripts', 'wplite_template_parts_styles', 10 );
function wplite_template_parts_styles(): void {
  $styles = [
    'sample' => [
      'version' => '1.0.0',
      'enqueue' => false,
  ...
  foreach ( $styles as $handle => $args ) :
    ...
    $suffix = $minified ? '.min' : '';
    $src = THEME_DIR_URI . "/template-parts/{$handle}/{$handle}.css";
